...
# Frontend: dominio y certificado (opcional)
variable "hosted_zone_id" {
  description = "ID de la Hosted Zone de Route53 para el dominio del frontend"
  type        = string
  default     = ""
}

variable "create_app_certificate" {
  description = "Crear certificado ACM para el dominio del frontend (us-east-1)"
  type        = bool
  default     = false
}

variable "app_certificate_arn" {
  description = "ARN del certificado ACM a usar en CloudFront (opcional)"
  type        = string
  default     = ""
}