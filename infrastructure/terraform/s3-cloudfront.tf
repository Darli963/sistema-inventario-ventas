...
resource "aws_s3_bucket_server_side_encryption_configuration" "private_bucket_encryption" {
  ...
  rule {
    apply_server_side_encryption_by_default {
      ...
      kms_master_key_id = aws_kms_key.kms_s3_private.arn
    }
...
resource "aws_cloudfront_distribution" "frontend_distribution" {
  ...
  # Alias opcional para dominio personalizado
  aliases = var.domain_name != null && trimspace(var.domain_name) != "" ? [var.domain_name] : null
  ...
  # Certificado para HTTPS: usar ACM si hay dominio, sino el default
  dynamic "viewer_certificate" {
    for_each = (var.app_certificate_arn != "" || (var.create_app_certificate && var.domain_name != null && trimspace(var.domain_name) != "")) ? [1] : []
    content {
      acm_certificate_arn      = var.app_certificate_arn != "" ? var.app_certificate_arn : aws_acm_certificate.app_cert[0].arn
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Condition = {
          StringNotEquals = {
            "s3:x-amz-server-side-encryption-aws-kms-key-id" = aws_kms_key.kms_s3_private.arn
          },
          Null = {
